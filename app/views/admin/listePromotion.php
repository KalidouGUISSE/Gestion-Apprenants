    ...
    <section class="content view-list">
      <div class="content-container view-list">
        ...
        <div class="stat-cards-container view-list">
          <div class="stat-card">
            <div class="stat-info">
              <h1><?= $nbApprenants ?? 0 ?> apprenant<?= ($nbApprenants ?? 0) !== 1 ? 's' : '' ?></h1>
              <p>Apprenants</p>
            </div>
            <div class="stat-icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <path fill="" d="m12 18.692l-6-3.261v-4.846L3.077 9L12 4.154L20.923 9v6.385h-1V9.562L18 10.585v4.846zm0-5.992L18.83 9L12 5.3L5.17 9zm0 4.852l5-2.7v-3.717l-5 2.708l-5-2.708v3.717zm0-3.487"/>
              </svg>
            </div>
          </div>
          ...
          <div class="stat-card">
            <div class="stat-info">
              <h1><?= $nbReferentiels ?? 0 ?></h1>
              <p>Référentiels</p>
            </div>
            <div class="stat-icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <path fill="" d="M2 4.75C2 3.784 2.784 3 3.75 3h4.971c.58 0 1.12.286 1.447.765l1.404 2.063a.25.25 0 0 0 .207.11h6.224c.966 0 1.75.783 1.75 1.75v.117H5.408a.848.848 0 0 0 0 1.695h15.484a1 1 0 0 1 .995 1.102L21 19.25c-.106 1.05-.784 1.75-1.75 1.75H3.75A1.75 1.75 0 0 1 2 19.25z" />
              </svg>
            </div>
          </div>
              ...
              <?php if (!empty($promosPagines)): ?>
                <?php foreach ($promosPagines as $promotion): ?>
                  <tr>
                    ...
                    <td><?= htmlspecialchars($promotion['idnom'] ?? 'N/A') ?></td>
                    <td><?= htmlspecialchars($promotion['debutAnnee'] ?? '-') ?></td>
                    <td><?= htmlspecialchars($promotion['finAnnnee'] ?? '-') ?></td>
                    <td>
                      <?php foreach ($promotion['referentiels'] ?? [] as $ref): ?>
                        <?php
                          $refName = strtolower($ref['nom']);
                        ...
                        ?>
                        <span class="badge <?= $badgeClass ?>"><?= htmlspecialchars($ref['nom']) ?></span>
                      <?php endforeach; ?>
                    </td>
                    <td>
                      <?php
